...
# Palantir-inspired theme: flat dark navy surfaces, hairline borders, electric
# blue primary, amber for alert/risk indicators. No glow, no gradients — the
# look is operational, not atmospheric. Palette is kept in sync with
# .streamlit/config.toml (primary = _BLUE).
_BG = "#0d1117"
_SURFACE = "#161d29"
_SURFACE_HI = "#1c2533"
_BORDER = "#2a3441"
...
st.markdown(
    f"""
    <style>
    /* Flat operational base — no gradients, no glow. */
    html, body, .stApp,
    [data-testid="stMain"],
    ...
    /* Sidebar is a distinct surface with a hairline right border. */
    [data-testid="stSidebar"] {{
        background: {_SURFACE} !important;
        border-right: 1px solid {_BORDER};
    }}

    /* Headings: uppercase, tight tracking, no glow. Subtitle in muted blue. */
    h1 {{
    ...
    /* Captions in muted slate — secondary annotation, not primary text. */
    [data-testid="stCaptionContainer"], .stCaption, small {{
        color: {_MUTED} !important;
        font-size: 0.78rem !important;
    }}

    /* Metric "tiles": flat panels with hairline borders, monospace values. */
    ...
    [data-testid="stMetricDelta"] {{ font-family: {_MONO} !important; }}
    ...
    /* Buttons: sharp rectangles, single accent on hover. */
    .stButton > button {{
        background: {_SURFACE};
        color: {_TEXT};
        border: 1px solid {_BORDER};
    ...
    /* Inputs / selects / sliders pick up the same hairline language. */
    ...
    /* Tabs: minimal underline-on-active, no pill background. */
    ...
    /* DataFrames / tables: monospace cells, hairline grid. */
    [data-testid="stDataFrame"] {{
        border: 1px solid {_BORDER};
        border-radius: 2px;
    }}

    /* Alerts: amber for warnings, blue for info, no rounded balloons. */
    [data-testid="stAlert"] {{
        border-left: 2px solid {_BLUE};
        border-radius: 2px;
        background: {_SURFACE};
    }}
    [data-baseweb="notification"][kind="warning"],
    [data-testid="stAlert"][kind="warning"] {{
        border-left-color: {_AMBER} !important;
    }}

    /* Links: blue, no underline by default. */
    a, a:visited {{ color: {_BLUE}; text-decoration: none; }}
    a:hover {{ text-decoration: underline; }}

    /* Code and monospace data values. */
    code, pre, kbd {{
        font-family: {_MONO} !important;
        background: {_SURFACE} !important;
        color: {_BLUE};
